...
//! 超過分（1 回最大 128 行）の古い行を越える時刻)。署名済み envelope の作成時刻が W 未満の投稿は索引の trigger が拒否し、W 未満の撤回
...
/// 受入下限 W を進めて返す。W は現在 − T まで上げ、容量を超えていれば超過分（最大 `budget` 件）の古い行を越える
/// ところまで一度に上げる。古い行は各表の時刻の索引の先頭から最大 `budget` 件ずつだけ読む。
pub async fn advance_retention_floor(pool: &PgPool, now: i64, budget: usize) -> Result<i64> {
    let Some((floor, units, capacity, retention_secs)) =
        sqlx::query_as::<_, (i64, i64, Option<i64>, Option<i64>)>(
            "SELECT floor, units, capacity, retention_secs FROM cn_index.retention_state",
        )
        .fetch_optional(pool)
        .await?
    else {
    ...
    let (Some(capacity), Some(retention_secs)) = (capacity, retention_secs) else {
        return Ok(floor);
    };
    let mut target = floor.max(now - retention_secs);
    let excess = units - capacity;
    if excess > 0 {
        let oldest: Vec<i64> = sqlx::query_scalar(
            "SELECT t FROM (
                 (SELECT created_at AS t FROM cn_index.index_entries
                  WHERE created_at >= $1 ORDER BY created_at LIMIT $2)
                 UNION ALL
                 (SELECT created_at FROM cn_index.known_post_withdrawals
                  WHERE created_at >= $1 ORDER BY created_at LIMIT $2)
                 UNION ALL
                 (SELECT created_at FROM cn_index.relation_actions
                  WHERE created_at >= $1 ORDER BY created_at LIMIT $2)
                 UNION ALL
                 (SELECT EXTRACT(EPOCH FROM updated_at)::BIGINT FROM cn_safety.scan_verdicts
                  WHERE updated_at >= TO_TIMESTAMP($1) ORDER BY updated_at LIMIT $2)
                 UNION ALL
                 (SELECT EXTRACT(EPOCH FROM completed_at)::BIGINT FROM cn_safety.content_scan_cache
                  WHERE completed_at >= TO_TIMESTAMP($1) ORDER BY completed_at LIMIT $2)
             ) AS oldest ORDER BY t LIMIT $2",
        )
        .bind(target)
        .bind(excess.min(i64::try_from(budget)?))
        .fetch_all(pool)
        .await?;
    ...
    Ok(sqlx::query_scalar(
        "UPDATE cn_index.retention_state SET floor = GREATEST(floor, $1) RETURNING floor",
    )
    .bind(target)
    .fetch_one(pool)
    .await?)
