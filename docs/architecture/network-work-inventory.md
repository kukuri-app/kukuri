...
| N10 `rebuild_author_relationships/current_mutual_direct_message_peers/schedule_direct_message_reconcile`（R4-Dで撤去） | — | 関係は読むときに対象authorのfollow edgeを主キーで点読（`get_author_relationship`）。関係cache tableと全件再計算は無い | 1件の関係の読取りは履歴件数に依存しない | NW-8、mutual未確認時拒否 |
| N11 `reconcile_direct_message_subscriptions/spawn_direct_message_subscription/direct_message_topic_snapshot`（R4-Dで撤去） | — | DMの受信はN66のaccount route、送信・再送はaccount共通owner。pairwise topicの購読taskは無い | 起動時は再送ownerの起動だけ | NW-8/9、ACK/tombstone |
...
| N22 `load_private_epoch_snapshot`、`private_control_support.rs` | epoch参加/handoff受取り、1秒間隔・全体10秒 | caller取消/期限で停止。metadata・policy・ownerと自分の参加recordだけを手元の次に書き手とchannel gossip scopeの最大4 providerからkey指定で読む | #1221 R5-Cで全参加者の反復と旧50ms polling・sync再開を撤去 | NW-9、旧epoch grant経路 |
...
R4-D（2026-09-26）で旧pairwise受信・送信・ACKを撤去し、DMはN66のaccount routeだけで送受信する。
