...
- AC-3: 同一の level・target・message が**連続した**場合、ring buffer 上で 1 行に集約され、件数・最初と
  最後の時刻が残る。relay の drop 警告は本文が定数で連続して出るため、この集約が効き、直前の文脈を
  押し出さない(次に発生したときの原因特定の材料になる)。
  - 効かない場合: 2 種類の警告が交互に出る場合や、`remote_fetch` のように field の値が行ごとに変わる
    警告は集約されない。後者の抑制は #1207 と #1223 の範囲。
  - 1 行の上限(4KiB)で切り詰めた後の本文で比較するため、先頭 4KiB と全長が同じ別々の行は
    1 行にまとまる。影響は開発者ログの表示に限られる。
- TR-1 / TR-2 の回帰保護: 受信側 runtime の全 worker を止めている間は転送が 1 本も完了せず(停止が
  効いていることの確認)、再開後に relay 経由の転送がすべて完了する。drop 件数は build の最適化と
  CPU の空きに左右されるため、test の合否には使わず出力だけする。
