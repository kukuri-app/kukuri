...
| AC-3 | 列を閉じても参加(live・Dome)の購読は続く。live 退出は参加の holder だけを外す。`unsubscribe_topic` は参加を止めない | `leaving_a_screen_and_ending_participation_are_separate` |
...
| AC-9 | private channel の参加は参加の holder で channel key。上限なら状態を保存しない。退出で外す | `participation_over_the_limit_is_refused_without_saving_state` |
...
- app-api: `service/scope_leases.rs`(`ScopeLeases`・`ScopeKey`・`ScopeLimitReached`・holder 名・`stop_scope_task`)。
  ...
  private channel の epoch が変わったら task を現 epoch へ作り直す(直前の epoch の replica は閉じない。参加者が handoff の grant を
  同期で受け取るため。旧 sync の撤去は R5-H)。Dome の移動は、次の段が projection から読む staging を自分で projection へ置く
...
- app-api `cargo test -p kukuri-app-api --lib` 510 件成功、`--features iroh-integration-tests` 557 件成功。
  `scope_leases.rs` の 9 件(AC-1〜AC-6・AC-9)。
...
- mutation check(外すと失敗することを確認し、元に戻した): 上限の判定(AC-1 の 3 件)、timeline の読込での購読の開始(AC-2)、
  最後の holder での hint・replica の後始末(AC-2)、`unsubscribe_topic` が参加も外す(AC-3)、起動時の block 相手の購読(AC-4)、
