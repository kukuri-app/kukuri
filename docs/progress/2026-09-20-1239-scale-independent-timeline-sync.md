...
原因: 自分の replica のプロフィールの索引の補完が購読の開始と同時に起動し、その書き込みが、起動直後の stack の作り直しと重なると、
新しい stack の docs の actor が 2 秒以上応答せず、健全性の確認が時間切れになる（補完を起動しない、印を書かないなどの変更で失敗が 0 回になることで切り分けた）。

修正: 自分の replica の背景の仕事（自分の edge の読み出しとプロフィールの索引の補完）は、購読の開始から 1 秒後の最初の tick で起動する（`own_replica_work.rs` の `START_DELAY`）。
件数に比例する処理の頻度を下げる対処ではなく、起動直後の stack の作り直しと重ならないようにする順序の調整である。修正後は 25 回中 0 回。
