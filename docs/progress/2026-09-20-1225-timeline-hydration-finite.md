...
- 修正後は、変化の無い走査が約 1/12 になり、頻度も最大 30 秒に 1 回まで下がる。反映を伴う走査は、起動時と実際に変化があったときだけになる。

### 訂正（2026-09-20）
当初はここで「現時点では topic 全体で 1 replica のまま足りる。10 万件規模になったら再検討する」と結論したが、誤りとして撤回する。
件数の規模で「足りる」と判断しないこと、取りこぼしゼロの全件取得を目標にしないことが設計原則である（`AGENTS.md` の「設計原則: 件数に依存しない処理」）。

本 Issue の修正は発火の頻度と 1 回あたりの定数倍を下げただけで、次の欠陥が残っている。解消は #1239 が所有する。
- 全件走査は、発火のたびに replica の全 entry を読み込んでメモリに載せる（総件数に比例）。契機は、購読タスクの起動時、recovery tick、購読の再起動、hint、利用者の操作。
- 指紋が 1 件でも変わると、全行を deserialize して全行を書き直す。活発な topic ほど指紋は毎回変わり、省略が効かない。
- repost・reaction・reply・bookmark は、走査の完了を待ってから実行される。件数が増えるほど操作が遅くなる。
- 全件走査という仕組み自体が「取りこぼしを全件比較で埋める」発想に基づいている。
